...
function SignInButton({ onGoogleLoginSuccess }) {
  ...
  return (
    ...
    <div className="min-h-screen w-full flex bg-slate-50 font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 relative overflow-hidden items-center justify-center p-16 text-white">
        {/* Background Visuals */}
        <div className="absolute top-0 left-0 w-full h-full opacity-10">
          <div className="absolute top-10 left-10 w-64 h-64 rounded-full bg-white blur-3xl mix-blend-overlay animate-pulse"></div>
          <div className="absolute bottom-10 right-10 w-96 h-96 rounded-full bg-indigo-300 blur-3xl mix-blend-overlay"></div>
        </div>

        {/* Hero Content */}
        <div className="relative z-10 max-w-lg space-y-8">
          <div className="bg-white/10 backdrop-blur-md inline-flex items-center gap-3 px-4 py-2 rounded-full border border-white/20 shadow-inner">
            <Sparkles className="w-5 h-5 text-yellow-300" />
            <span className="text-sm font-medium tracking-wide">AI 驅動的智能繪圖助手</span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xl text-indigo-100 font-light leading-relaxed opacity-90">
              將您的想法瞬間轉化為專業圖表。
              <br />
              專注於邏輯，剩下的交給 AI。
            </p>
          ...
          <div className="space-y-4 pt-4">
            <div className="flex items-center gap-4 text-indigo-50">
              ...
              <div>
                <h3 className="font-semibold text-white">智能生成</h3>
                <p className="text-sm opacity-75">輸入文字描述，自動產出對應圖表</p>
              </div>
            </div>
            <div className="flex items-center gap-4 text-indigo-50">
              <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center border border-white/10">
              ...
              <div>
                <h3 className="font-semibold text-white">即時預覽</h3>
                <p className="text-sm opacity-75">即時編輯與渲染，支援多種圖表類型</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 relative">
        <div className="w-full max-w-md space-y-8 bg-white p-8 sm:p-10 rounded-3xl shadow-xl border border-slate-100 relative overflow-hidden">
          {/* Top Design Accent */}
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-500 to-violet-500"></div>

          <div className="text-center space-y-2">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-indigo-50 mb-4 shadow-sm border border-indigo-100 transform rotate-3 hover:rotate-6 transition-transform">
              <Wand2 className="w-8 h-8 text-indigo-600" />
            </div>
            <h2 className="text-3xl font-bold text-slate-800 tracking-tight">歡迎回來</h2>
            <p className="text-slate-500">請登入以繼續使用 Mermaid Flow</p>
          </div>

          <div className="space-y-4 pt-4">
            <button
              onClick={handleMicrosoftLogin}
              className="w-full flex items-center justify-center gap-3 bg-[#2F2F2F] hover:bg-black text-white h-12 px-6 rounded-xl font-medium transition-all duration-200 shadow-md hover:shadow-lg active:scale-[0.98] group"
            >
              <LogIn className="w-5 h-5 group-hover:text-indigo-300 transition-colors" />
              <span className="tracking-wide">Microsoft 帳號登入</span>
            </button>

            <div className="relative flex py-2 items-center">
              <div className="flex-grow border-t border-slate-200"></div>
              <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-medium uppercase tracking-wider">或者是</span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>

            <button
              onClick={() => googleLogin()}
              className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 h-12 px-6 rounded-xl font-medium transition-all duration-200 shadow-sm hover:shadow-md active:scale-[0.98] group"
            >
          ...
          <div className="pt-6 text-center">
            <p className="text-xs text-slate-400">
              繼續即表示您同意 <a href="#" className="text-indigo-600 hover:underline">服務條款</a> 與 <a href="#" className="text-indigo-600 hover:underline">隱私權政策</a>
            </p>
            <p className="text-[10px] text-slate-300 mt-4">Version 1.2.0 • Mermaid Flow</p>
          </div>
        </div>
      </div>
    </div>
...
function MainApp({ user, onLogout }) {
  ...
  // Save UI State
  const [saveStatus, setSaveStatus] = useState('idle'); // idle, generating_name, user_input, saving
  const [generatedName, setGeneratedName] = useState('');

  const saveToWorkspace = async (e) => {
    e?.preventDefault();
    setSaveStatus('generating_name');
    ...
    setGeneratedName(defaultName);
    setSaveStatus('user_input');
  };

  const confirmSave = async (finalName) => {
      ...
      {saveStatus !== 'idle' && (
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center animate-in fade-in">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4 border border-indigo-100">
            {saveStatus === 'generating_name' && (
              <div className="flex flex-col items-center py-4">
                <Loader2 className="w-10 h-10 text-indigo-600 animate-spin mb-3" />
                <h3 className="text-lg font-bold text-slate-800">AI 正在命名中</h3>
                <p className="text-sm text-slate-500 mt-1">根據圖表內容產生標題...</p>
              </div>
            )}

            {saveStatus === 'user_input' && (
              <div className="flex flex-col gap-4">
                <div className="text-center">
                  <div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-3">
                    <Save className="w-6 h-6 text-indigo-600" />
                  </div>
                  <h3 className="text-lg font-bold text-slate-800">儲存圖表</h3>
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase mb-1 block">圖表名稱</label>
                  <input
                    type="text"
                    value={generatedName}
                    onChange={(e) => setGeneratedName(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-slate-800"
                    autoFocus
                    onKeyDown={(e) => e.key === 'Enter' && confirmSave(generatedName)}
                  />
                </div>
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setSaveStatus('idle')} className="flex-1 py-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-medium transition-colors">取消</button>
                  <button onClick={() => confirmSave(generatedName)} className="flex-1 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium transition-colors shadow-sm">確認儲存</button>
                </div>
              </div>
            )}
            ...
            {saveStatus === 'saving' && (
              <div className="flex flex-col items-center py-4">
                <Loader2 className="w-10 h-10 text-indigo-600 animate-spin mb-3" />
                <h3 className="text-lg font-bold text-slate-800">正在儲存</h3>
                <p className="text-sm text-slate-500 mt-1">寫入雲端資料庫...</p>
              </div>
            )}
          </div>
        </div>
